...
int main(int argc, char* argv[]) {
	if(argc < 3) {
		fprintf(stderr, "Usage: %s <report_file_name> <signing_public_key>\n", argv[0]);
		return -1;
	}
	Crypto *m_crypto = new Crypto(); 		
	Attestation *m_attestation = new Attestation(m_crypto);
	if (m_crypto == NULL || m_attestation == NULL) {
		fprintf(stderr, "Crypto + Attestation could not be instantiated\n");
		return -1;
	} 
	m_attestation->attest_remote_report(NULL, 0, NULL, 0);
	return 0;
}
